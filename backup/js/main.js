...
// js_valid_rating
    ...
    if ($("#jsmainbanner").length) {
        const blink1 = jsmainbanner.querySelector(".blink1"),
            blink2 = jsmainbanner.querySelector(".blink2"),
            maxMove = jsmainbanner.offsetWidth / 2,
        ...
        function getMousePos(xRef, yRef) {
            let panelRect = jsmainbanner.getBoundingClientRect();
            return {
                x: Math.floor(xRef - panelRect.left) /(panelRect.right-panelRect.left)*jsmainbanner.offsetWidth,
                y: Math.floor(yRef - panelRect.top) / (panelRect.bottom -panelRect.top) * jsmainbanner.offsetHeight
            };
        }
